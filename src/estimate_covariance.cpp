...
*/
bool updateNecessaryTransform(const autoware_msgs::DetectedObjectArray &input,
                              tf::StampedTransform &local2global_)
{
    bool success = true;
    tf::TransformListener tf_listener_;
    try {
        tf_listener_.waitForTransform(input.header.frame_id, "map",
                                      ros::Time(0), ros::Duration(1.0));
        tf_listener_.lookupTransform("map", input.header.frame_id,
...
void get_logfilename()
{
    ...
    logfile_name_ += ".csv";
    logfile.open("~/RSU/Experiment/2022/" + logfile_name_, std::ofstream::out | std::ofstream::app);
...
void RSU_callback(const autoware_msgs::DetectedObjectArray& input)
{
    ...
    for(int i = 0; i < input.objects.size(); i++){
        ...
        if(distance < distance_threshold_){ // write data into file
            ...
            logfile << to_string(ground_truth.position.x) << ", " << to_string(ground_truth.position.y) << ", " << to_string(tf::getYaw(ground_truth.orientation)) << ", ";
            logfile << to_string(out_pose.position.x) << ", " << to_string(out_pose.position.y) << ", " << to_string(tf::getYaw(out_pose.orientation)) << ", ";
            logfile << to_string(input.objects[i].dimensions.x) << ", " << to_string(input.objects[i].dimensions.y) << ", " << to_string(input.objects[i].dimensions.z) << ", ";
...
int main(int argc, char **argv)
{
    ...
    n.getParam("RSU_topic_name", RSU_topic_name_);
